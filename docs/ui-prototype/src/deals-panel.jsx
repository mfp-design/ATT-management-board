...
export function DealsPanel({deals, setDeals, revenue, setRevenue, accounts, businesses, members, stages, begin, focusDeal, setFocusDeal, go, saveGuard}) {
  ...
  const update = (id, values, reason) => setDeals(xs => xs.map(x => x.id === id ? {...x, ...values, history: [...(x.history || []), {before: `${x.stage} / ${yen(x.amount)} / ${x.business} / ${x.member}`, after: `${values.stage || x.stage} / ${yen(values.amount ?? x.amount)} / ${values.business || x.business} / ${values.member || x.member}`, reason, at: stamp()}]} : x));
  const edit = d => begin({title: d ? '案件を編集' : '案件を登録', confirm: true, fields: [
    ...
    input('account', '取引先', 'select', accounts.filter(a => !a.mergedInto).map(a => `${a.id} / ${a.name}`), {value: d ? accounts.filter(a => !a.mergedInto).find(a => a.id === d.accountId || a.name === d.account) && `${accounts.filter(a => !a.mergedInto).find(a => a.id === d.accountId || a.name === d.account).id} / ${d.account}` : ''}),
    ...
    save: f => {
      const account = accounts.find(a => f.account.startsWith(a.id + ' /'));
  ...
  if (selected) {
    ...
    return <><button className="text-btn" onClick={() => setFocusDeal(null)}>← 案件一覧へ戻る</button><Card title={d.name} subtitle={`${d.business} · ${d.member}`} action={!d.cancelled && <button onClick={() => edit(d)}>案件を編集</button>}>
      <dl className="details"><dt>取引先</dt><dd>{d.account}</dd><dt>案件段階</dt><dd><Badge>{d.cancelled ? '取消' : d.stage}</Badge></dd><dt>案件金額</dt><dd>{yen(d.amount)}</dd><dt>共同担当者</dt><dd>{d.coMembers?.join('、') || 'なし'}</dd><dt>成約予定日</dt><dd>{d.due}</dd><dt>売上計上予定日</dt><dd>{d.revenueDate}</dd><dt>メモ</dt><dd>{d.memo || 'なし'}</dd></dl>
    ...
    </Card><Card title="案件の変更履歴"><GridTable headers={['変更前', '変更後', '理由・日時']} rows={(d.history || []).map(h => [h.before, h.after, <>{h.reason}<small>{h.at}</small></>])}/></Card></>;
  }
